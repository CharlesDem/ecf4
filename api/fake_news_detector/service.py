...
from fake_news_detector.preprocessing import clean_for_tfidf
...
class FakeNewsDetectorService:
    ...
    def _predict_proba(self, titles: list[str]) -> np.ndarray:
        cleaned_titles = [clean_for_tfidf(title) for title in titles]
        vectors = self.vectorizer.transform(cleaned_titles)
